...
class Themify_Customizer {
	...
	function __construct() {
		...
		add_action( 'wp_enqueue_scripts' , array( $this, 'enqueue_fonts' ), 14 );
	...
	function enqueue_fonts() {
		...
		if ( ! empty( $this->customizer_fonts ) ) {
			...
			wp_enqueue_style( 'customizer-google-fonts' . md5( $customizer_fonts . $subsets ), themify_https_esc( 'http://fonts.googleapis.com/css' ) . '?family=' . $customizer_fonts . '&subset=' . $subsets );
		}
	}
